---
name: felo-mindmap
description: "Generate mindmaps with Felo Mindmap API. Use when users ask to create/make/generate mindmaps, mind maps, or thinking maps. Handles API key check, mindmap creation with various layouts, and final mindmap_url output."
...
**Durable config (all platforms):** Add to `~/.hermes/.env` (or `~/AppData/Local/hermes/.env` on Windows):

```
FELO_API_KEY=fk-...
```

The `.env` approach survives restarts and gateway reconnects. Export-based env vars are
session-only and lost when the gateway restarts.
...
- **The script path is relative to the skill directory.** On Windows, Hermes home is `~/AppData/Local/hermes/`, so the full path is:
  `~/AppData/Local/hermes/skills/felo/felo-mindmap/scripts/run_mindmap_task.mjs`
  If `felo-mindmap/scripts/run_mindmap_task.mjs` doesn't resolve, use the full `HERMES_HOME`-relative path or `cd` to the skill dir first.

## Windows-Specific Quirks

- The Felo install script (`install-hermes.sh`) installs to `~/.hermes/skills/felo/` by default, but on Windows, Hermes home is at `~/AppData/Local/hermes/`. If the script ran but skills aren't visible, copy them:
  ```bash
  cp -r ~/.hermes/skills/felo ~/AppData/Local/hermes/skills/felo
  ```
- Then reload with `hermes skills list` or use `/reload-skills` in-session.
